...
export function Fab({ onClick, label = "Add expense" }: FabProps) {
  ...
  return (
    <motion.button
      ...
      className="fixed z-40 bottom-[88px] right-5 lg:bottom-8 lg:right-8 h-14 w-14 rounded-full bg-primary text-on-primary shadow-product flex items-center justify-center outline-none"
    >
      <PlusIcon size={26} />
    </motion.button>
  );
