...
export function Fab({ onClick, label = "Add expense" }: FabProps) {
  ...
  return (
    <motion.button
      ...
      className="fixed z-40 right-5 bottom-[calc(env(safe-area-inset-bottom)+5.5rem)] lg:bottom-8 lg:right-8 h-14 w-14 rounded-full bg-primary text-on-primary shadow-product flex items-center justify-center outline-none"
    >
      <PlusIcon size={26} />
    </motion.button>
  );
