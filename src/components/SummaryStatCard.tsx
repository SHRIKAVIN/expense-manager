import type { ReactNode } from "react";
import { cn } from "@/lib/cn";

interface SummaryStatCardProps {
  label: string;
  children: ReactNode;
  className?: string;
  onPress?: () => void;
  "data-testid"?: string;
}

/** Elevated stat tile for dashboard summary grids. */
export function SummaryStatCard({
  label,
  children,
  className,
  onPress,
  "data-testid": testId,
}: SummaryStatCardProps) {
  const classes = cn(
    "flex w-full flex-col items-center justify-center gap-1 rounded-2xl bg-canvas px-3 py-3 text-center sm:px-4 sm:py-3.5",
    "shadow-[0_6px_20px_rgba(0,0,0,0.1),0_2px_8px_rgba(0,0,0,0.06)]",
    "dark:shadow-[0_6px_20px_rgba(0,0,0,0.4),0_2px_8px_rgba(0,0,0,0.25)]",
    onPress && "cursor-pointer outline-none active:scale-[0.98] transition-transform",
    className,
  );

  const inner = (
    <>
      <p className="text-[11px] font-bold leading-snug text-ink-muted-80 sm:text-[13px] text-center">{label}</p>
      <div className="min-w-0 text-[15px] font-bold tabular-nums leading-none tracking-tight sm:text-[22px] lg:text-[26px] text-center">
        {children}
      </div>
    </>
  );

  if (onPress) {
    return (
      <button type="button" data-testid={testId} onClick={onPress} className={classes}>
        {inner}
      </button>
    );
  }

  return (
    <div data-testid={testId} className={classes}>
      {inner}
    </div>
  );
}
