-- Partial / split reimbursements: payer gets request.amount; requester keeps
-- their share (expense.amount - request.amount) instead of excluding the whole row.
...
begin
  ...
  remaining := exp.amount - req.amount;
  if remaining <= 0.009 then
    ...
    update public.expenses
    set amount = remaining, updated_at = now_ts
    where id = req.expense_id and user_id = req.requester_id;
  end if;
